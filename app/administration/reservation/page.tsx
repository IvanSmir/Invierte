        ...
        <table className="min-w-full border border-gray-200">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="px-4 py-2 text-left text-sm font-medium text-muted-foreground">Lote</th>
            </tr>
          </thead>
          <tbody>
            {reservations.map((res) => (
              <tr key={res.id} className="border-t border-gray-200">
                ...
                <td className="px-4 py-2 ">{res.User?.email || "Sin email"}</td>

                <td className="px-4 py-2 underline">
                  <Link href={`/marketplace/${res.lot?.propertyId}?lot=${res.lotId}`}>
                    Ver reserva
                  </Link>
                </td>
              </tr>
